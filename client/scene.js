import * as THREE from "three";
import { GLTFLoader } from "../vendor/GLTFLoader.js";
import { buildRoadGeometry, buildBoostPadGeometry, buildStartLineGeometry, sample, closest, ROAD_HALF_WIDTH } from "./track.js";

export const KART_COLORS = { red: 0xe23d2e, blue: 0x2f6fe0, green: 0x3fa04c, yellow: 0xf2b21c };
export const KART_IDS = ["red", "blue", "green", "yellow"];

// 3-step gradient map shared by every toon material
function gradientMap() {
  const data = new Uint8Array([90, 180, 255, 255]);
  const tex = new THREE.DataTexture(data, 4, 1, THREE.RedFormat);
  tex.needsUpdate = true;
  tex.minFilter = tex.magFilter = THREE.NearestFilter;
  return tex;
}
const GRAD = gradientMap();

export function toon(color, opts = {}) {
  return new THREE.MeshToonMaterial({ color, gradientMap: GRAD, ...opts });
}

// Canvas-generated placeholder textures, replaced by generated assets when present.
function canvasTexture(draw, size = 256) {
  const c = document.createElement("canvas");
  c.width = c.height = size;
  draw(c.getContext("2d"), size);
  const tex = new THREE.CanvasTexture(c);
  tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
  tex.colorSpace = THREE.SRGBColorSpace;
  return tex;
}

async function loadTextureOrFallback(url, fallbackDraw) {
  const loader = new THREE.TextureLoader();
  try {
    const tex = await loader.loadAsync(url);
    tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
    tex.colorSpace = THREE.SRGBColorSpace;
    return tex;
  } catch {
    console.warn(`asset missing, using placeholder: ${url}`);
    return canvasTexture(fallbackDraw);
  }
}

const roadDraw = (g, s) => {
  g.fillStyle = "#6b6f78"; g.fillRect(0, 0, s, s);
  g.fillStyle = "#767a84";
  for (let i = 0; i < 60; i++) g.fillRect(Math.random() * s, Math.random() * s, 3, 3);
  g.fillStyle = "#f6e7b2"; g.fillRect(0, 0, 6, s); g.fillRect(s - 6, 0, 6, s);
};
const grassDraw = (g, s) => {
  g.fillStyle = "#7ec850"; g.fillRect(0, 0, s, s);
  g.fillStyle = "#8fd45e";
  for (let i = 0; i < 80; i++) g.fillRect(Math.random() * s, Math.random() * s, 4, 2);
};
const dirtDraw = (g, s) => {
  g.fillStyle = "#c9a06a"; g.fillRect(0, 0, s, s);
  g.fillStyle = "#b8905c";
  for (let i = 0; i < 50; i++) g.fillRect(Math.random() * s, Math.random() * s, 5, 3);
};
const checkerDraw = (g, s) => {
  const n = 8, c = s / n;
  for (let y = 0; y < n; y++) for (let x = 0; x < n; x++) {
    g.fillStyle = (x + y) % 2 ? "#111" : "#fff";
    g.fillRect(x * c, y * c, c, c);
  }
};

// Chibi placeholder kart: big yellow head, colored body, rounded kart shell.
// Swapped for generated GLBs (assets/kart_<id>.glb) when they exist.
function placeholderKart(colorHex) {
  const grp = new THREE.Group();
  const outline = [];
  const add = (geo, mat, x, y, z, withOutline = true) => {
    const m = new THREE.Mesh(geo, mat);
    m.position.set(x, y, z);
    grp.add(m);
    if (withOutline) outline.push(m);
    return m;
  };
  const body = toon(colorHex);
  const dark = toon(0x2a2a2e);
  const skin = toon(0xf2c94c); // warm yellow skin per style formula

  add(new THREE.BoxGeometry(1.5, 0.5, 2.2), body, 0, 0.45, 0);                       // shell
  add(new THREE.BoxGeometry(1.1, 0.25, 0.9), dark, 0, 0.65, 0.2, false);             // seat
  const wheelGeo = new THREE.CylinderGeometry(0.34, 0.34, 0.3, 12);
  for (const [wx, wz] of [[-0.8, 0.75], [0.8, 0.75], [-0.8, -0.75], [0.8, -0.75]]) {
    const w = new THREE.Mesh(wheelGeo, dark);
    w.rotation.z = Math.PI / 2;
    w.position.set(wx, 0.34, wz);
    grp.add(w);
  }
  add(new THREE.CylinderGeometry(0.28, 0.36, 0.55, 10), body, 0, 0.95, -0.1);        // torso
  const head = add(new THREE.SphereGeometry(0.52, 16, 12), skin, 0, 1.65, -0.1);     // oversized chibi head
  const eyeW = new THREE.SphereGeometry(0.13, 8, 8);
  const eyeB = new THREE.SphereGeometry(0.055, 6, 6);
  const white = new THREE.MeshBasicMaterial({ color: 0xffffff });
  const black = new THREE.MeshBasicMaterial({ color: 0x111111 });
  for (const ex of [-0.18, 0.18]) {
    const w = new THREE.Mesh(eyeW, white); w.position.set(ex, 1.7, 0.35); grp.add(w);
    const p = new THREE.Mesh(eyeB, black); p.position.set(ex, 1.7, 0.46); grp.add(p);
  }
  // inverted-hull outlines (cheap toon edge, karts only)
  const outMat = new THREE.MeshBasicMaterial({ color: 0x1c1c22, side: THREE.BackSide });
  for (const m of outline) {
    const o = new THREE.Mesh(m.geometry, outMat);
    o.position.copy(m.position);
    o.rotation.copy(m.rotation);
    o.scale.setScalar(1.06);
    grp.add(o);
  }
  const o = new THREE.Mesh(head.geometry, outMat);
  o.position.copy(head.position); o.scale.setScalar(1.06); grp.add(o);
  return grp;
}

// generated meshes face arbitrary directions; measured per model (front must point +z)
const KART_YAW_FIX = { red: 0, blue: -Math.PI / 2, green: 0, yellow: Math.PI / 2 };

async function loadKartModel(id) {
  const loader = new GLTFLoader();
  try {
    const gltf = await loader.loadAsync(`./assets/kart_${id}.glb`);
    const model = gltf.scene;
    // retint into toon shading, keep any baked textures as maps
    model.traverse((n) => {
      if (n.isMesh) {
        const old = n.material;
        n.material = new THREE.MeshToonMaterial({
          color: old.color ?? 0xffffff, map: old.map ?? null, gradientMap: GRAD,
        });
      }
    });
    model.rotation.y = KART_YAW_FIX[id] ?? 0;
    const wrap = new THREE.Group();
    wrap.add(model);
    const box = new THREE.Box3().setFromObject(wrap); // after rotation: length is along z
    const s = 2.4 / (box.max.z - box.min.z || 1);
    model.scale.setScalar(s);
    model.position.y = -box.min.y * s;
    return wrap;
  } catch {
    console.warn(`kart model missing, using placeholder: kart_${id}.glb`);
    return placeholderKart(KART_COLORS[id]);
  }
}

export async function createScene(canvas, seededRandom) {
  const renderer = new THREE.WebGLRenderer({ canvas, antialias: true });
  renderer.setClearColor(0x9ed4f5); // sunny pastel sky
  const scene = new THREE.Scene();
  scene.fog = new THREE.Fog(0x9ed4f5, 120, 320);

  const camera = new THREE.PerspectiveCamera(70, 1, 0.1, 500);
  camera.position.set(0, 30, -40);

  scene.add(new THREE.HemisphereLight(0xfff6e0, 0x7ec850, 1.1));
  const sun = new THREE.DirectionalLight(0xfff2cc, 1.6);
  sun.position.set(60, 100, 40);
  scene.add(sun);

  const [roadTex, grassTex, dirtTex] = await Promise.all([
    loadTextureOrFallback("./assets/tex_road.png", roadDraw),
    loadTextureOrFallback("./assets/tex_grass.png", grassDraw),
    loadTextureOrFallback("./assets/tex_dirt.png", dirtDraw),
  ]);
  grassTex.repeat.set(60, 60);

  const geos = buildRoadGeometry();
  scene.add(new THREE.Mesh(geos.road, toon(0xffffff, { map: roadTex })));
  const dirtMat = toon(0xffffff, { map: dirtTex });
  scene.add(new THREE.Mesh(geos.shoulderL, dirtMat));
  scene.add(new THREE.Mesh(geos.shoulderR, dirtMat));
  const ground = new THREE.Mesh(new THREE.PlaneGeometry(500, 500), toon(0xffffff, { map: grassTex }));
  ground.rotation.x = -Math.PI / 2;
  ground.position.y = -0.02;
  scene.add(ground);

  const padMat = new THREE.MeshBasicMaterial({ color: 0xff3fa4, transparent: true, opacity: 0.85 });
  for (const g of buildBoostPadGeometry()) scene.add(new THREE.Mesh(g, padMat));

  const checker = canvasTexture(checkerDraw, 128);
  checker.repeat.set(1, 1);
  scene.add(new THREE.Mesh(buildStartLineGeometry(), new THREE.MeshBasicMaterial({ map: checker })));

  // trackside props: generated GLBs if present, toon primitives otherwise
  await scatterProps(scene, seededRandom);

  // drift/boost sparks: one Points pool for everything
  const SPARKS = 150;
  const sparkGeo = new THREE.BufferGeometry();
  const sparkPos = new Float32Array(SPARKS * 3).fill(0);
  const sparkCol = new Float32Array(SPARKS * 3).fill(0);
  sparkGeo.setAttribute("position", new THREE.BufferAttribute(sparkPos, 3));
  sparkGeo.setAttribute("color", new THREE.BufferAttribute(sparkCol, 3));
  const sparks = new THREE.Points(sparkGeo, new THREE.PointsMaterial({ size: 0.35, vertexColors: true, transparent: true, opacity: 0.9 }));
  sparks.frustumCulled = false;
  scene.add(sparks);
  const sparkLife = new Float32Array(SPARKS).fill(0);
  const sparkVel = new Float32Array(SPARKS * 3).fill(0);
  let sparkHead = 0;

  const karts = {}; // seat -> {group, id}

  const api = {
    renderer, scene, camera,
    fov: 70,

    async addKart(seat, id) {
      if (karts[seat]) scene.remove(karts[seat].group);
      const group = await loadKartModel(id);
      scene.add(group);
      karts[seat] = { group, id };
    },
    removeKart(seat) {
      if (karts[seat]) { scene.remove(karts[seat].group); delete karts[seat]; }
    },
    hasKart(seat) { return !!karts[seat]; },

    setKartPose(seat, x, z, yaw, visual = {}) {
      const k = karts[seat];
      if (!k) return;
      k.group.position.set(x, 0, z);
      const slide = visual.drifting ? (visual.driftDir ?? 0) * 0.35 : 0;
      k.group.rotation.set(0, yaw + slide, 0);
      k.group.rotation.z = -(visual.steer ?? 0) * 0.08;
      const t = performance.now() / 1000;
      k.group.position.y = Math.abs(Math.sin(t * 10 + seat)) * 0.03 * Math.min(1, (visual.speed ?? 0) / 10);
    },

    emitSparks(x, z, yaw, tier) {
      const col = tier >= 2 ? [1, 0.25, 0.64] : tier === 3 ? [1, 0.8, 0.2] : [0.4, 0.8, 1];
      for (let i = 0; i < 3; i++) {
        const j = sparkHead = (sparkHead + 1) % SPARKS;
        sparkPos[j * 3] = x - Math.sin(yaw) * 1.2 + (Math.random() - 0.5) * 0.6;
        sparkPos[j * 3 + 1] = 0.2;
        sparkPos[j * 3 + 2] = z - Math.cos(yaw) * 1.2 + (Math.random() - 0.5) * 0.6;
        sparkCol.set(col, j * 3);
        sparkVel[j * 3] = (Math.random() - 0.5) * 3;
        sparkVel[j * 3 + 1] = 2 + Math.random() * 2;
        sparkVel[j * 3 + 2] = (Math.random() - 0.5) * 3;
        sparkLife[j] = 0.4;
      }
    },

    updateSparks(dt) {
      for (let j = 0; j < SPARKS; j++) {
        if (sparkLife[j] <= 0) { sparkPos[j * 3 + 1] = -10; continue; }
        sparkLife[j] -= dt;
        sparkPos[j * 3] += sparkVel[j * 3] * dt;
        sparkPos[j * 3 + 1] += sparkVel[j * 3 + 1] * dt;
        sparkPos[j * 3 + 2] += sparkVel[j * 3 + 2] * dt;
        sparkVel[j * 3 + 1] -= 9 * dt;
      }
      sparkGeo.attributes.position.needsUpdate = true;
      sparkGeo.attributes.color.needsUpdate = true;
    },

    // spring-damped chase camera
    camPos: new THREE.Vector3(0, 6, -12),
    followKart(seat, boosting, dt) {
      const k = karts[seat];
      if (!k) return;
      const yaw = k.group.rotation.y;
      const target = new THREE.Vector3(
        k.group.position.x - Math.sin(yaw) * 8,
        4.2,
        k.group.position.z - Math.cos(yaw) * 8,
      );
      const stiff = 1 - Math.exp(-6 * dt);
      this.camPos.lerp(target, stiff);
      camera.position.copy(this.camPos);
      const look = k.group.position.clone();
      look.y = 1.2;
      look.x += Math.sin(yaw) * 4;
      look.z += Math.cos(yaw) * 4;
      camera.lookAt(look);
      const targetFov = boosting ? 80 : 70;
      this.fov += (targetFov - this.fov) * Math.min(1, 8 * dt);
      camera.fov = this.fov;
      camera.updateProjectionMatrix();
    },

    overviewCamera(t) {
      camera.position.set(Math.sin(t * 0.08) * 120, 70, Math.cos(t * 0.08) * 120);
      camera.lookAt(0, 0, 0);
      camera.fov = 60;
      camera.updateProjectionMatrix();
    },

    resize(w, h, dpr) {
      renderer.setPixelRatio(dpr);
      renderer.setSize(w, h, false);
      camera.aspect = w / h;
      camera.updateProjectionMatrix();
    },

    render() { renderer.render(scene, camera); },
    drawCalls() { return renderer.info.render.calls; },
  };
  return api;
}

async function scatterProps(scene, rand) {
  const loader = new GLTFLoader();
  const outMat = new THREE.MeshBasicMaterial({ color: 0x1c1c22, side: THREE.BackSide });

  const placeholders = {
    prop_tree: () => {
      const g = new THREE.Group();
      const trunk = new THREE.Mesh(new THREE.CylinderGeometry(0.35, 0.45, 2, 8), toon(0x8a5a33));
      trunk.position.y = 1;
      const crown = new THREE.Mesh(new THREE.SphereGeometry(1.9, 12, 10), toon(0x4fae53));
      crown.position.y = 3.2;
      const o = new THREE.Mesh(crown.geometry, outMat);
      o.position.copy(crown.position); o.scale.setScalar(1.05);
      g.add(trunk, crown, o);
      return g;
    },
    prop_house: () => {
      const g = new THREE.Group();
      const base = new THREE.Mesh(new THREE.BoxGeometry(5, 3.4, 4.4), toon(0xf7cfd8));
      base.position.y = 1.7;
      const roof = new THREE.Mesh(new THREE.ConeGeometry(4.2, 2.4, 4), toon(0xb2543e));
      roof.position.y = 4.6;
      roof.rotation.y = Math.PI / 4;
      g.add(base, roof);
      return g;
    },
    prop_donut: () => {
      const g = new THREE.Group();
      const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.25, 0.25, 6, 8), toon(0x9a9aa5));
      pole.position.y = 3;
      const donut = new THREE.Mesh(new THREE.TorusGeometry(2, 0.85, 12, 24), toon(0xf7a8c4));
      donut.position.y = 7.5;
      const o = new THREE.Mesh(donut.geometry, outMat);
      o.position.copy(donut.position); o.scale.setScalar(1.04);
      g.add(pole, donut, o);
      return g;
    },
  };

  const PROP_HEIGHT = { prop_tree: 6, prop_house: 5.5, prop_donut: 8 };

  async function make(id) {
    try {
      const gltf = await loader.loadAsync(`./assets/${id}.glb`);
      const m = gltf.scene;
      m.traverse((n) => {
        if (n.isMesh) n.material = new THREE.MeshToonMaterial({ color: n.material.color ?? 0xffffff, map: n.material.map ?? null, gradientMap: GRAD });
      });
      const box = new THREE.Box3().setFromObject(m);
      const s = (PROP_HEIGHT[id] ?? 5) / (box.max.y - box.min.y || 1);
      m.scale.setScalar(s);
      m.position.y = -box.min.y * s;
      const wrap = new THREE.Group();
      wrap.add(m);
      return wrap;
    } catch {
      return placeholders[id]();
    }
  }

  const templates = {
    prop_tree: await make("prop_tree"),
    prop_house: await make("prop_house"),
    prop_donut: await make("prop_donut"),
  };

  // one landmark donut sign near the start, trees/houses scattered off-road
  const donut = templates.prop_donut.clone();
  const s0 = sample(0.03);
  donut.position.set(s0.pos.x * 1.35, 0, s0.pos.z * 1.35);
  scene.add(donut);

  for (let i = 0; i < 26; i++) {
    const t = rand();
    const { pos, tan } = sample(t);
    const side = new THREE.Vector3().crossVectors(tan, new THREE.Vector3(0, 1, 0));
    const dist = 12 + rand() * 25;
    const sgn = rand() > 0.5 ? 1 : -1;
    const kind = rand() > 0.75 ? "prop_house" : "prop_tree";
    const p = templates[kind].clone();
    p.position.set(pos.x + side.x * dist * sgn, 0, pos.z + side.z * dist * sgn);
    if (Math.hypot(p.position.x, p.position.z) > 145) continue;
    // the circuit folds back on itself; make sure the spot is clear of every road section
    if (Math.abs(closest(p.position).lateral) < ROAD_HALF_WIDTH + 4) continue;
    p.rotation.y = rand() * Math.PI * 2;
    p.scale.setScalar(0.8 + rand() * 0.5);
    scene.add(p);
  }
}
